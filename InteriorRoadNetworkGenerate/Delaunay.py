...
from math import radians,acos,sin,cos,asin,sqrt
...
def dist(p1, p2):
    ...
    # degree to radian
    # lon1, lat1, lon2, lat2 = map(radians, [p1[0], p1[1],p1[0],p2[1]])
    # # haversine equation
    # dlon = lon2 - lon1
    # dlat = lat2 - lat1
...
def delaunay(points):
    ...
    for triangle in triangles:
        ind=[]
        for i in range(0,3):
            ind.append(points.index((triangle[i][0],triangle[i][1])))
        simplex.append(ind)
    neighbors=[]
    for i in range(len(simplex)):
        ...
        for j in range(len(simplex)):
            if i!=j:
                if len(set(simplex[i]).intersection(set(simplex[j])))==2:
                    neighbor.append(j)
        while len(neighbor)!=3:
            neighbor.append(-1)
        neighbors.append(neighbor)

    return simplex,neighbors
